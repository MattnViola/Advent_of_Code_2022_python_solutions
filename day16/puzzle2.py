# Find the most effiecient path for 1 person in 26 mins, 
# then find the most efficient path for the 2nd person using the visited list from the first person.
# There will be corner cases with other inputs where you will need to be more thorough,
# A clear way would to be to evaluate the best 2nd person path for every 1st person path, then find the highest value for each pair
...
best_path_value = (0, None)
def main():
    global best_path_value

    with open('Advent_of_Code_2022_python_solutions\day16\input.txt','r') as file:
        for line in file:
    ...
    print(len(Valve.valves))

    # Use BFS to find the shortest way to get to each positive flow valve from each valve
    for valve in Valve.positive_valves:
        valve.pos_valve_distance()

    simulate_paths(26, Valve.valves['AA'])
    x, best_path_value = best_path_value, (0, None)
    simulate_paths(26, Valve.valves['AA'], visited=x[1])
    print(x[0] + best_path_value[0])
...
def best_path(x, visited):
    global best_path_value
    if x > best_path_value[0]:
        best_path_value = (x, visited.copy())
    return
